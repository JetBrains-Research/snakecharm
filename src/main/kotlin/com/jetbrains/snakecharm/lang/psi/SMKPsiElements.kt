...
class SMKRule(node: ASTNode): SmkRuleLike<SMKRuleParameterListStatement>(node) {
...
class SMKCheckPoint(node: ASTNode) : SmkRuleLike<SMKRuleParameterListStatement>(node) {
    override fun acceptPyVisitor(pyVisitor: PyElementVisitor) = when (pyVisitor) {
